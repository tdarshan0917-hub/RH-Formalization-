#!/bin/zsh
cat > RHFormalization/DefaultZeroExhaustion.lean <<'EOF'
...
EOF
lake build RHFormalization.DefaultZeroExhaustion 2>&1 | tee zex_a.log | grep -e "error" -e "depends on axioms" -e "Build completed"
if grep -q "error" zex_a.log; then
  echo "FAILED -> removing (errors below)"
  grep -B3 -A16 "error" zex_a.log | head -100
  rm RHFormalization/DefaultZeroExhaustion.lean
  exit 1
fi
if ! grep -q "defaultZeroExhaustion' depends on axioms: \[propext, Classical.choice, Quot.sound\]" zex_a.log; then
