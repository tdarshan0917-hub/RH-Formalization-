#!/bin/zsh
cat > RHFormalization/DesignedDetailedConstruction.lean <<'EOF'
...
EOF
lake build RHFormalization.DesignedDetailedConstruction 2>&1 | tee dy_a.log | grep -e "error" -e "depends on axioms" -e "Build completed"
if grep -q "error" dy_a.log; then
  echo "FAILED -> removing (errors below)"
  grep -B3 -A14 "error" dy_a.log | head -90
  rm RHFormalization/DesignedDetailedConstruction.lean
  exit 1
fi
if ! grep -q "designedY" dy_a.log; then echo "EMPTY MODULE -> abort"; rm RHFormalization/DesignedDetailedConstruction.lean; exit 1; fi
