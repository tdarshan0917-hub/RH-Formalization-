#!/bin/zsh
echo "===== 0. sum-holomorphy name probe ====="
cat > SumHoloProbe.lean <<'EOF'
...
EOF
lake env lean SumHoloProbe.lean 2>&1 | head -8
echo "===== 1. install HPPStabilization ====="
cat > RHFormalization/HPPStabilization.lean <<'EOF'
import RHFormalization.PairPoleIsolation
...
theorem zetaStripRegion_mono {m n : ℕ} (h : m ≤ n) :
  ...
  · rw [mem_closedBall_zero_iff] at hball ⊢
    linarith
  · simp only [Set.mem_setOf_eq] at hre1 ⊢
    linarith
...
theorem zeroPolePartial_stabilized
  ...
  rw [hd1, hd2, div_add_div_same, pairGroupedPoleClass_coeff]
  push_cast
  ring
...
EOF
lake build RHFormalization.HPPStabilization 2>&1 | tee stab_a.log | grep -e "error" -e "depends on axioms" -e "Build completed"
if grep -q "error" stab_a.log; then
  echo "FAILED -> removing (errors below)"
  grep -B2 -A12 "error" stab_a.log | head -90
  rm RHFormalization/HPPStabilization.lean
  exit 1
fi
if ! grep -q "zeroPolePartial_stabilized' depends on axioms: \[propext, Classical.choice, Quot.sound\]" stab_a.log; then
