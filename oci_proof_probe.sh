#!/bin/zsh
echo "===== 1. MeromorphicOnC ground truth ====="
grep -rn "MeromorphicOnC" RHFormalization/*.lean | grep -e "def " -e "abbrev " | head -5
echo "===== 2. ConnectedOmegaAPI fields + preconnected lemmas in project ====="
grep -rn -A4 "structure ConnectedOmegaAPI" RHFormalization/*.lean | head -10
grep -rn "IsPreconnected Ω\|isPreconnected_Omega\|IsConnected Ω" RHFormalization/*.lean | head -8
echo "===== 3. battery: hinge names ====="
cat > OCIBattery.lean <<'EOF'
...
EOF
lake env lean OCIBattery.lean 2>&1
echo "===== 4. THE PROOF (scratch, hpre as parameter) ====="
cat > OCIProofProbe.lean <<'EOF'
import RHFormalization.OmegaCodiscreteIdentityFromNormalForms
...
example (hpre : IsPreconnected Ω)
  ...
  have hmem : {z | f z = g z} ∈ Filter.codiscreteWithin Ω := by
    ...
    have hz' : f z = g z := by
      have : f z - g z = 0 := by simpa [Pi.sub_apply, Pi.zero_apply] using hz
      linarith_or_polyrith : sorry
...
EOF
python3 - <<'PY'
# fix the placeholder line: replace the sorry stub with sub_eq_zero
p='OCIProofProbe.lean'
s=open(p).read()
s=s.replace("""      have : f z - g z = 0 := by simpa [Pi.sub_apply, Pi.zero_apply] using hz
      linarith_or_polyrith : sorry""",
"""      have h : f z - g z = 0 := by simpa [Pi.sub_apply, Pi.zero_apply] using hz
      exact sub_eq_zero.mp h""")
open(p,'w').write(s)
print("probe file finalized")
PY
lake env lean OCIProofProbe.lean 2>&1
