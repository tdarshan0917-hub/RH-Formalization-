...
-/
import RHFormalization.DecodedGalerkinOperatorDFiniteStage
import RHFormalization.DecodedGalerkinStageShift
import Mathlib

-- SENTINEL: decoded-op-nonneg-discharge-v1
...
theorem decodedFreeDiagOp_form_eq_sum (μ : Fin N → ℝ) (y : EuclideanSpace ℂ (Fin N)) :
...
theorem decodedGalerkinOpCLM_form_split
    (μ : Fin N → ℝ) (δ : ℝ) (qs : Finset ℕ) (w : ℕ → ℝ) (L : ℝ) (M : ℝ)
    (y : EuclideanSpace ℂ (Fin N)) :
    RCLike.re (inner ℂ y (decodedGalerkinOpCLM μ δ qs w L M y))
      = RCLike.re (inner ℂ y (perturbedOp μ (decodedGalerkinVC (N := N) δ qs w L) y))
        + M * ‖y‖ ^ 2 := by
  have happly : decodedGalerkinOpCLM μ δ qs w L M y
      = perturbedOp μ (decodedGalerkinVC (N := N) δ qs w L) y + (M : ℂ) • y := by
    show (Matrix.toEuclideanCLM (𝕜 := ℂ) (n := Fin N)
        (perturbedMatrix μ (decodedGalerkinVC (N := N) δ qs w L)
          + (M : ℂ) • (1 : Matrix (Fin N) (Fin N) ℂ))) y
      = Matrix.toEuclideanLin (perturbedMatrix μ (decodedGalerkinVC (N := N) δ qs w L)) y
        + (M : ℂ) • y
...
theorem decodedGalerkinOpCLM_nonneg_of_shift
    (μ : Fin N → ℝ) (δ : ℝ) (qs : Finset ℕ) (w : ℕ → ℝ) (L : ℝ) (M : ℝ)
    (hM : ∀ k, growthDrop (decodedGalerkinVC (N := N) δ qs w L) ≤ μ k + M) :
    ∀ y : EuclideanSpace ℂ (Fin N),
      0 ≤ RCLike.re (inner ℂ y (decodedGalerkinOpCLM μ δ qs w L M y)) := by
  intro y
  rw [decodedGalerkinOpCLM_form_split]
  have hlow := perturbedOp_formLowerBound μ (decodedGalerkinVC (N := N) δ qs w L) y
  have hflip1 := re_inner_comm (perturbedOp μ (decodedGalerkinVC (N := N) δ qs w L)) y
  ...
  have hfree : RCLike.re (inner ℂ y (freeDiagOp μ y)) = ∑ k, μ k * ‖y k‖ ^ 2 :=
    decodedFreeDiagOp_form_eq_sum μ y
  have hnormsum : ‖y‖ ^ 2 = ∑ k, ‖y k‖ ^ 2 := by
    rw [PiLp.norm_sq_eq_of_L2]
  have hterm : 0 ≤ ∑ k,
      (μ k - growthDrop (decodedGalerkinVC (N := N) δ qs w L) + M) * ‖y k‖ ^ 2 := by
    apply Finset.sum_nonneg
    intro k _
    exact mul_nonneg (by linarith [hM k]) (sq_nonneg _)
  have hexpand : ∑ k,
      (μ k - growthDrop (decodedGalerkinVC (N := N) δ qs w L) + M) * ‖y k‖ ^ 2
      = (∑ k, μ k * ‖y k‖ ^ 2)
        - growthDrop (decodedGalerkinVC (N := N) δ qs w L) * (∑ k, ‖y k‖ ^ 2)
        + M * (∑ k, ‖y k‖ ^ 2) := by
...
noncomputable def decodedGalerkinOperatorDFiniteStage_ofShift
    (n : ℕ) (μ : Fin N → ℝ) (δ : ℝ) (qs : Finset ℕ) (w : ℕ → ℝ) (L : ℝ) (M : ℝ)
    (hshift : ∀ k, growthDrop (decodedGalerkinVC (N := N) δ qs w L) ≤ μ k + M) :
    DFiniteStage :=
  decodedGalerkinOperatorDFiniteStage n μ δ qs w L M
    (decodedGalerkinOpCLM_nonneg_of_shift μ δ qs w L M hshift)

#print axioms decodedFreeDiagOp_form_eq_sum
#print axioms decodedGalerkinOpCLM_form_split
#print axioms decodedGalerkinOpCLM_nonneg_of_shift
#print axioms exists_shift_making_decoded_galerkin_nonneg
#print axioms decodedGalerkinOperatorDFiniteStage_ofShift
