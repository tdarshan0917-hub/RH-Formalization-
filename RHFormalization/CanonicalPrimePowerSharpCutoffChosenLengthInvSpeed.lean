...
/-!
# RHFormalization.CanonicalPrimePowerSharpCutoffChosenLengthInvSpeed

Chosen-length inverse-speed theorem for the sharp-cutoff window package.

This closes the `h_inv_speed_tendsto_zero` input needed by
`chosenLengthWindowAPI_of_invSpeed` and the selected D-side `WindowFR` constructor.
-/
...
/--
For the chosen-length sharp-cutoff package, the inverse compact-window speed
tends to zero on every compact real set.

This is the remaining rate input needed to convert the compact-speed API into
the qualitative `DCanonicalWindowAPI`.
-/
theorem chosenLength_inv_speed_tendsto_zero
