...
/-!
# RHFormalization.CanonicalPrimePowerSharpCutoffChosenLengthOverlapBuilder

Chosen-length Appendix-D overlap builder.

This file specializes the generic finite-stage split + pointwise limits theorem
to the chosen-length sharp-cutoff prime-power route, assuming the F/R limit data
use the same chosen alpha sequence as the B-side finite-canonical limit.
-/
...
/--
Chosen-length D overlap builder from the generic stage-split limit theorem,
assuming F/R use the same chosen alpha sequence as the chosen-length B-limit.

This supplies the `overlapBuilder` input needed by

`buildDDetailedConstructionWithOperatorLegalityFromSharpCutoffChosenLengthMassEnvelope`.
-/
theorem chosenLengthOverlapBuilder_of_alpha_aligned
