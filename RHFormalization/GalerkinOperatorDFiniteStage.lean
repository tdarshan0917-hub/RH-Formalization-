...
import RHFormalization.GalerkinNativeStageAssembly
...
* `native := galerkinNativeStage μ δ qs w L M hnn`
* `appendixDFiniteFStage := galerkinPerturbedFStage (N := N) μ δ qs w L`
...
-/
noncomputable def galerkinOperatorDFiniteStage
    (n : ℕ)
    (μ : Fin N → ℝ)
...
-/
theorem galerkinOperatorDFiniteStage_F_stage
    (n : ℕ)
    (μ : Fin N → ℝ)
    (δ : ℝ) (qs : Finset ℕ) (w : ℕ → ℝ) (L : ℝ)
    (M : ℝ)
    (hnn :
      ∀ y : EuclideanSpace ℂ (Fin N),
        0 ≤ RCLike.re (inner ℂ y (galerkinOpCLM μ δ qs w L M y))) :
    (galerkinOperatorDFiniteStage n μ δ qs w L M hnn).appendixDFiniteFStage =
      galerkinPerturbedFStage (N := N) μ δ qs w L := by
  rfl

/--
The cutoff is inherited from `primePowerStage`.
-/
theorem galerkinOperatorDFiniteStage_R
    (n : ℕ)
    (μ : Fin N → ℝ)
    (δ : ℝ) (qs : Finset ℕ) (w : ℕ → ℝ) (L : ℝ)
    (M : ℝ)
    (hnn :
      ∀ y : EuclideanSpace ℂ (Fin N),
        0 ≤ RCLike.re (inner ℂ y (galerkinOpCLM μ δ qs w L M y))) :
    (galerkinOperatorDFiniteStage n μ δ qs w L M hnn).R = (n : ℝ) + 1 := by
  rfl

#print axioms galerkinOperatorDFiniteStage
#print axioms galerkinOperatorDFiniteStage_F_stage
#print axioms galerkinOperatorDFiniteStage_R
