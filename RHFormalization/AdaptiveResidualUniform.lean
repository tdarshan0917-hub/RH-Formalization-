...
/-!
# AdaptiveResidualUniform — the DISP row for the LIVE galerkinVC object

Clone of DecodedResidualUniform onto galerkinVC (bridge impossible;
mechanism V-independent). Uses: banked adaptivePerturbedLam (live),
trace_RD_V_RH_V_RD engine (V-generic), galerkinVC_column_norm_sq_le,
eigenvalue floor, reciprocal trick, schedule collapse (verbatim clone).
Consumer: h_disp_le of SECT-v2.
-/
...
theorem adaptiveSecondResolventResidual_norm_le
    (c : ℝ) (n : ℕ) (s : ℂ) {δ C₀ : ℝ} (hδ : 0 < δ) (hC₀ : 0 ≤ C₀)
    (hlow : ∀ i, δ ≤ ‖s + (SupVConst : ℂ)
        + ((adaptivePerturbedLam c n i : ℝ) : ℂ)‖)
    (hbound : ∀ lam : ℝ, 0 ≤ lam →
  ...
  have hlow' : ∀ i : Fin (adaptiveN c n), δ ≤ ‖s + (SupVConst : ℂ)
    ...
    first
      | exact h
      | (unfold adaptivePerturbedLam at h; exact h)
      | simpa [adaptivePerturbedLam] using h
  ...
  have hcol : ∀ m : Fin (adaptiveN c n),
    ...
    fun m => galerkinVC_column_norm_sq_le (admR n) (adaptiveL c n) hL m
  ...
  unfold adaptiveSecondResolventResidual
  rw [norm_mul, hdens]
  refine mul_le_mul_of_nonneg_left ?_ (le_of_lt (one_div_pos.mpr h2L))
  exact le_trans htr
    (mul_le_mul_of_nonneg_left hsum (le_of_lt (inv_pos.mpr hδ)))

/-- Schedule collapse (verbatim clone — V-independent arithmetic). -/
theorem adaptive_residual_schedule_collapse (c : ℝ) (n : ℕ)
    {C₀ : ℝ} (hC₀ : 0 < C₀) :
    1 / (2 * adaptiveL c n) *
        (C₀ * (C₀ ^ 2 * adaptiveL c n *
          ((adaptiveN c n : ℝ)
...
/-- **THE DISP ROW**: uniform boundedness of the LIVE second-resolvent
residual on Ω-compacts. -/
theorem adaptiveSecondResolventResidual_uniform_bound (c : ℝ)
    (K : Set ℂ) (hK : IsCompact K) (hKO : K ⊆ Ω) :
    ∃ C : ℝ, 0 < C ∧ ∀ (n : ℕ), ∀ s ∈ K,
      ‖adaptiveSecondResolventResidual c n s‖ ≤ C := by
  ...
  have heig : ∀ i, 0 ≤ adaptivePerturbedLam c n i := by
    intro i
    first
      | exact adaptivePerturbedLam_nonneg c n i
      | (unfold adaptivePerturbedLam
         first
           | exact admissibleEigenvalue_nonneg (adaptiveL c n) (adaptiveL_pos c n) _ _
               (fun k => galerkinFreeMu_nonneg _ _ k) i
           | exact admissibleEigenvalue_nonneg (adaptiveL c n) (adaptiveL_pos c n)
               (fun k => galerkinFreeMu_nonneg _ _ k) i
           | (refine admissibleEigenvalue_nonneg (adaptiveL c n) (adaptiveL_pos c n) _ _
                 (fun k => ?_) i
              first
                | exact galerkinFreeMu_nonneg _ _ k
                | (unfold galerkinFreeMu; positivity)
                | exact sq_nonneg _)
           | exact admissibleEigenvalue_nonneg (adaptiveL c n) (adaptiveL_pos c n) i
           | exact admissibleEigenvalue_nonneg (adaptiveL c n) (adaptiveL_pos c n) _ i)
  ...
  have hlow : ∀ i, 1 / C₀ ≤ ‖s + (SupVConst : ℂ)
      + ((adaptivePerturbedLam c n i : ℝ) : ℂ)‖ := by
    intro i
    set z : ℂ := s + (SupVConst : ℂ)
        + ((adaptivePerturbedLam c n i : ℝ) : ℂ) with hz
    have hzfloor : δ₀ ≤ ‖z‖ := by
      have hnn : (0:ℝ) ≤ SupVConst + adaptivePerturbedLam c n i :=
        add_nonneg SupVConst_nonneg_adm (heig i)
      have h := hδ₀ s hs (SupVConst + adaptivePerturbedLam c n i) hnn
      have hcast : s + ((SupVConst + adaptivePerturbedLam c n i : ℝ) : ℂ)
          = z := by
    ...
    have hinvle : ‖z‖⁻¹ ≤ C₀ := by
      have h1 := hbound' (adaptivePerturbedLam c n i) (heig i)
      rw [← hz] at h1
      rw [norm_inv] at h1
      have hle1 : (1 + adaptivePerturbedLam c n i)⁻¹ ≤ 1 := by
        have h11 : (1:ℝ) ≤ 1 + adaptivePerturbedLam c n i := by
      ...
      calc ‖z‖⁻¹ ≤ C₀ * (1 + adaptivePerturbedLam c n i)⁻¹ := h1
  ...
  have hmain := adaptiveSecondResolventResidual_norm_le c n s hδpos hC₀pos.le
      hlow hbound'
  have hinv : (1 / C₀)⁻¹ = C₀ := by rw [one_div, inv_inv]
  rw [hinv] at hmain
  exact le_trans hmain (adaptive_residual_schedule_collapse c n hC₀pos)

#print axioms adaptiveSecondResolventResidual_norm_le
#print axioms adaptive_residual_schedule_collapse
#print axioms adaptiveSecondResolventResidual_uniform_bound
