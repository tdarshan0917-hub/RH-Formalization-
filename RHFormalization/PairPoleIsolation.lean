...
theorem pairPole_isolated (W : ZeroWitness) :
  ...
  have heq : a * (1 - a) = W.ρ * (1 - W.ρ) := neg_injective h1
  have hzero : (a - W.ρ) * (a - (1 - W.ρ)) = 0 := by
    linear_combination -heq
  obtain ⟨_, hre0, hre1⟩ := W.h_zero
  have hkill : ∀ (b : ℂ), b.re < 1 → a = b →
      (∀ k, x (φ k) ≠ b) → False := by
    intro b hb hab hxb
    rw [hab] at htend
    have hA : AnalyticAt ℂ riemannZeta b :=
      zeta_analyticOnNhd_re_lt_one b hb
    rcases hA.eventually_eq_zero_or_eventually_ne_zero with hz | hne
    · exact zeta_not_locally_zero b hb hz
    · have hne' : ∀ᶠ z in nhds b, z ≠ b → riemannZeta z ≠ 0 := by
        simpa [eventually_nhdsWithin_iff] using hne
      have hk := htend.eventually hne'
      obtain ⟨k, hkk⟩ := hk.exists
      exact hkk (hxb k) (hx_zero (φ k)).1
  rcases mul_eq_zero.mp hzero with h | h
  · have hab : a = W.ρ := sub_eq_zero.mp h
    exact hkill W.ρ hre1 hab (fun k => hx_ne1 (φ k))
  · have hab : a = 1 - W.ρ := sub_eq_zero.mp h
    have hb1 : (1 - W.ρ).re < 1 := by
      simp only [Complex.sub_re, Complex.one_re]; linarith
    exact hkill (1 - W.ρ) hb1 hab (fun k => hx_ne2 (φ k))
