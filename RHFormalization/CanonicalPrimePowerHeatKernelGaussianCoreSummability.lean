...
/-!
# RHFormalization.CanonicalPrimePowerHeatKernelGaussianCoreSummability

Closes summability of the Gaussian-core envelope using eventual domination
by the already-proved NatValue majorant.
-/
...
lemma valid_primePower_natValue_lt_finite (N : ℕ) :
  ...
  have hm_le_pm : m ≤ p ^ m := by
    have hm_le_two : m ≤ 2 ^ m := nat_le_two_pow m
...
lemma eventually_not_valid_or_natValue_ge (N : ℕ) :
...
theorem heatKernelGaussianCoreEnvelope_summable
  ...
  filter_upwards [eventually_not_valid_or_natValue_ge N] with q hq
