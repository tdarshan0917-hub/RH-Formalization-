import RHFormalization.DecodedGalerkinFullToDiagonalDuhamelSplit
...
ROUTE CARD
1. `decodedPairedDuhamel1Integrand`: the T_a-paired order-1 Duhamel integrand
   `Tr(D(t−u)·V·D_exp(u)·T_a)` — the object whose spike sum produces the
   arithmetic one-letter package (the P^can of ONE-STEP′′).
2. `decodedGalerkinPairedSandwich_trace_split`: the paired Duhamel integrand
...
-- SENTINEL: decoded-paired-sandwich-split-v2
...
noncomputable def decodedPairedDuhamel1Integrand
...
theorem decodedGalerkinPairedSandwich_trace_split
    ...
    (Matrix.diagonal (fun m : Fin N => heatWeight (N := N) L (t - u) m)
        ...
        * NormedSpace.exp (u • (-(galerkinK (N := N) L
            + decodedGalerkinV (N := N) δ qs w L)))
        * galerkinT (N := N) L a).trace
      = - decodedPairedDuhamel1Integrand (N := N) δ qs w L t u a
        + (Matrix.diagonal (fun m : Fin N => heatWeight (N := N) L (t - u) m)
            * (-(decodedGalerkinV (N := N) δ qs w L))
            * (∫ s in (0:ℝ)..u,
  ...
  have hsplit := decodedGalerkinFullSandwich_split (N := N) δ qs w L t u
...
#print axioms decodedPairedDuhamel1Integrand
#print axioms decodedGalerkinPairedSandwich_trace_split
