...
print(f"regenerated with coeff-tail fix: {len(body)} chars")
PY
if [ $? -ne 0 ]; then echo "PATCH FAILED"; rm -f RHFormalization/ReflectionPairPoleClass.lean; exit 1; fi
lake build RHFormalization.ReflectionPairPoleClass 2>&1 | tee pair_b.log | grep -e "error" -e "depends on axioms" -e "Build completed"
if grep -q "error" pair_b.log; then
  echo "FAILED -> removing (errors below)"
  grep -B3 -A14 "error" pair_b.log | head -60
  rm RHFormalization/ReflectionPairPoleClass.lean
  exit 1
fi
if ! grep -q "pairGroupedPoleClass_coeff' depends on axioms: \[propext, Classical.choice, Quot.sound\]" pair_b.log; then
