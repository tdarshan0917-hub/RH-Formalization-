#!/bin/zsh
echo "===== 0. PrimePowerPair shape (for the toPP inhabitant) ====="
grep -rn -B2 -A14 "structure PrimePowerPair" RHFormalization/*.lean | head -20
grep -rn "instance.*PrimePowerPair\|Inhabited PrimePowerPair\|def.*: PrimePowerPair :=" RHFormalization/*.lean | head -5
echo "===== 1. install EmptyDFiniteStage ====="
...
noncomputable def emptyDFiniteStage : DFiniteStage :=
  { L := 1
    ...
    h_resolventBoundConstant_nonneg := by intro s _; le_refl 0 |> exact
...
if grep -q "error" empty_a.log; then
  echo "FAILED -> removing (errors below; section 0 above shows PrimePowerPair for the fix)"
