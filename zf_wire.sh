#!/bin/zsh
cat > RHFormalization/FinalSpineFromRealZeroFree.lean <<'EOF'
...
EOF
echo "import RHFormalization.FinalSpineFromRealZeroFree" >> RHFormalization.lean
lake build 2>&1 | tail -20 | tee zf_wire_build.log
grep -n -e "error" -e "sorry" -e "depends on axioms" zf_wire_build.log
