...
# 1. guard the field: insert validity hypothesis before "q.center ≤ R →" inside h_mem_belowCutoff
fi=None
...
# 2. insert helper lemma before toMassBoundData def
di=None
for i,l in enumerate(lines):
...
# back up over the docstring comment block to insert before it
ins=di
k=di-1
while k>=0 and (lines[k].strip().startswith("/--") or lines[k].strip().endswith("-/") or (lines[k].strip() and not lines[k].strip().startswith("def") and k>di-8)):
...
# 3. replace the h_weightC_mass_le_of_center_le proof block
si=None
for i,l in enumerate(lines):
...
# 4. spike-pair-code call site: thread validity argument
p2='RHFormalization/AppendixDSpikePairCodeFromCutoff.lean'
t=open(p2).read()
if "Enum.h_mem_belowCutoff R q hqcenter" not in t: print("E6: call site not found"); sys.exit(1)
t=t.replace("Enum.h_mem_belowCutoff R q hqcenter","Enum.h_mem_belowCutoff R q hqvalid hqcenter")
open(p2,'w').write(t)
print("edits applied")
PY
...
echo "===== sanity: show the guarded field ====="
sed -n '/h_mem_belowCutoff :/,/belowCutoff R$/p' RHFormalization/CanonicalPrimePowerCutoffMassEnumeration.lean | head -8
