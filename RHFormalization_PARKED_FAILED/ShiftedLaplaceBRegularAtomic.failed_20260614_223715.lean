...
This repairs the previous failed version:
- do not `unfold shiftedLaplaceSqrt` after the kernel has already unfolded;
- do not use `.const_mul` on `HolomorphicAtC`, since `HolomorphicAtC` is an
  existential/analytic-at wrapper in this project.

The theorem here proves holomorphy of the shifted/Laplace kernel term assuming
the shifted square-root branch is holomorphic and nonzero at the point.
-/
  ...
  change
    HolomorphicAtC
      (fun s : ℂ =>
        q.weightC *
          ((1 : ℂ) / (2 * shiftedLaplaceSqrt s) *
            Complex.exp (-(q.center : ℂ) * shiftedLaplaceSqrt s)))
      z
