...
This repair fixes the sign-normal-form mismatch in the exponent:
Lean normalized `-a * sqrt` as `-(a * sqrt)`, so we prove holomorphy of
`a * shiftedLaplaceSqrt` first, then apply `.neg`.
-/
...
theorem shiftedLaplaceHeatKernelC_holomorphicAt_of_shiftedSqrt
  ...
  have h_a_mul :
      HolomorphicAtC
        ...
        z := by
    simpa using h_a.mul h_sqrt

  have h_arg :
      HolomorphicAtC
        (fun s : ℂ => -((a : ℂ) * shiftedLaplaceSqrt s))
        z := by
    simpa using h_a_mul.neg
  ...
  simpa [shiftedLaplaceHeatKernelC, shiftedLaplaceSqrt,
    one_div, div_eq_mul_inv, neg_mul, mul_assoc] using h_prod
