...
Already banked:
  each weighted term
    q.weightC * shiftedLaplaceHeatKernelC q.center
  is holomorphic at z under the shifted-sqrt branch hypotheses.

This file lifts that atomic result to arbitrary finite prime-power sums and
the finite canonical prime-power package.

Next after this:
  branch lemma for shiftedLaplaceSqrt on Ω,
  then infinite-tsum/local-uniform lift.
-/
...
theorem shiftedLaplaceFinsetWeightedSum_holomorphicAt_of_shiftedSqrt
    ...
    HolomorphicAtC
      (fun s : ℂ =>
        ∑ q in I, q.weightC * shiftedLaplaceHeatKernelC q.center s)
      z := by
  classical
