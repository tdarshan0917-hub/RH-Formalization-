#!/bin/zsh
echo "===== 0. weighted envelope def (repair reference) ====="
sed -n '40,80p' RHFormalization/CanonicalPrimePowerHeatKernelWeightedSummabilityTarget.lean
echo "===== 1. install DesignedRCutoffS ====="
cat > RHFormalization/DesignedRCutoffS.lean <<'EOF'
...
EOF
lake build RHFormalization.DesignedRCutoffS 2>&1 | tee drs_a.log | grep -e "error" -e "depends on axioms" -e "Build completed"
if grep -q "error" drs_a.log; then
  echo "FAILED -> removing (errors below; section 0 has the envelope def)"
  grep -B2 -A12 "error" drs_a.log | head -70
